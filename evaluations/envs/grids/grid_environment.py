...
class GridRMEnv(RewardMachineEnv):
    ...
    def get_policy_data(self, num_episodes, epsilon, gamma):
        ...
        for ep in range(num_episodes):
            ...
            while not done:
                ...
                rewards.append(r)
                s_prime = tuple(self.obs)
                u_prime = self.current_u_id
                state = [s[0], s[1], rm_id]
                next_state = [s_prime[0], s_prime[1], rm_id]
                trajectory.append([state, a, r, next_state])
                s = s_prime
                u = u_prime
