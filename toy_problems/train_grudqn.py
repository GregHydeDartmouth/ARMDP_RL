...
for t in range(0, trials):
    ...
    print("\nSEED: {}".format(seed))
    dqn_agent = GRUDQN(state_space, len(actions), lr=1e-3, tau=0.01, seed=seed)
    trial_averages = []
    ...
    for i in range(0, 50000):
        ...
        h = dqn_agent.get_init_hidden_state()
        ep_reward = 0
        for j in range(0, max_steps):
            action, h = dqn_agent.act(state, h)
...
with open('data/rm_{}_grudqn_reward_averages.csv'.format(rm), 'w', newline='') as f:
    writer = csv.writer(f)
